...
def extract_previous_st(direct_value: Any, recent_form: Any) -> Optional[float]:
    """Direct column first; otherwise extract the newest usable ST from recent_form JSON."""
    direct = safe_float(direct_value)
    ...
    preferred_keys = (
        "previous_st", "prev_st", "start_timing", "st",
        "average_st", "avg_st",
    )

    def walk(obj: Any) -> Optional[float]:
        if isinstance(obj, dict):
            # Keep insertion/list order: recent_form is normally newest first.
            for key in preferred_keys:
                if key in obj:
                    parsed = safe_float(obj.get(key))
                    if parsed is not None and -0.20 <= parsed <= 1.00:
                        return parsed
            for key in ("previous_race", "last_race", "latest", "recent", "results", "races", "items", "data"):
                if key in obj:
                    parsed = walk(obj.get(key))
...
class Schema:
    entry_race_id: str
    entry_lane: str
    entry_previous_st: Optional[str]
    entry_recent_form: Optional[str]
...
def detect_schema() -> Schema:
    ...
    entry_recent_form = pick(ec, ["recent_form", "recent_results", "history", "past_results"])
    ...
    if not entry_race_id or not entry_lane or (not entry_previous_st and not entry_recent_form):
        raise RuntimeError(
            "v2_race_entries の必要列を特定できません。\n"
            f"columns={', '.join(ec)}\n"
            "previous_st相当列、またはrecent_form相当JSON列が必要です。"
        )
    ...
    return Schema(
        ...
        entry_recent_form=entry_recent_form,
...
def load_rows(s: Schema) -> List[Dict[str, Any]]:
    ...
    e_prev = f"e.{qi(s.entry_previous_st)}" if s.entry_previous_st else "null"
    e_recent = f"e.{qi(s.entry_recent_form)}" if s.entry_recent_form else "null"
...
def main() -> None:
    ...
    print("✅ feature_lab_previous_st_detail_pg.py VERSION 2026-07-20 st-detail-v2-recent-form", flush=True)
    ...
    print(
        "schema: "
        f"previous_st={s.entry_previous_st or 'recent_form JSON'} "
        f"recent_form={s.entry_recent_form or 'NONE'} lane={s.entry_lane} "
    ...
    for row in rows:
        ...
        pst = extract_previous_st(row.get("previous_st"), row.get("recent_form"))
