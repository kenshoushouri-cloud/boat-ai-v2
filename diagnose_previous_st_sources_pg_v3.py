        ...
        if "race_id" in cols(table):
            rows = fetch_all(
                f"""select distinct jsonb_object_keys(
                       case when jsonb_typeof(t.{col}::jsonb)='object'
                            then t.{col}::jsonb else '{{}}'::jsonb end
                     ) as key
                    from {table} t
                    join v2_races r on r.race_id=t.race_id
                    where r.race_date >= %s and r.race_date <= %s
...
def main() -> None:
    ...
    print("✅ diagnose_previous_st_sources_pg.py VERSION 2026-07-20 source-audit-v3", flush=True)
    ...
    sections = [
        ("discover_st_like_columns", discover_st_like_columns),
        ("sample_recent_form", sample_recent_form),
        ("sample_results", sample_results),
        ("sample_exhibition", sample_exhibition),
        ("discover_json_keys", discover_json_keys),
        ("same_racer_history_sample", same_racer_history_sample),
        ("focused_previous_st_sources", focused_previous_st_sources),
    ]
    for section_name, fn in sections:
        try:
            ...
            import traceback
            print(f"\n[SECTION ERROR] {section_name}: {type(exc).__name__}: {exc}", flush=True)
            traceback.print_exc()
