...
Railway Start Command:
    python backtest_multi_patterns.py
...
def _require_settings() -> None:
    print("✅ backtest_multi_patterns.py VERSION 2026-06-16", flush=True)
...
def _is_backtest_ready(result: Optional[Dict[str, Any]], entries: List[Dict[str, Any]], odds: Dict[str, float]) -> bool:
    ...
    if result.get("result_status") not in ("official", "parse_incomplete"):
        return False
    if not result.get("trifecta_ticket"):
        return False
...
STRATEGIES: List[Strategy] = [
    Strategy("all_1pt", "全候補・EV上位1点", seed_only=False, main_count=1),
    Strategy("all_2pt", "全候補・EV上位2点", seed_only=False, main_count=2),
    Strategy("seed_1pt", "シード戦限定・1頭固定1点", seed_only=True, first_fixed=1, main_count=1),
    Strategy("seed_2pt", "シード戦限定・1頭固定2点", seed_only=True, first_fixed=1, main_count=2),
    Strategy("seed_main1_ins1", "シード戦限定・本線1点＋保険1点", seed_only=True, first_fixed=1, main_count=1, insurance_count=1),
    Strategy("seed_main2_ins1", "シード戦限定・本線2点＋保険1点", seed_only=True, first_fixed=1, main_count=2, insurance_count=1),
    Strategy("seed_rule_v1", "シード戦限定・本線2点＋保険6倍以上", seed_only=True, first_fixed=1, main_count=2, insurance_count=1, insurance_min_odds=INSURANCE_MIN_ODDS),
]
...
def _build_day_candidates(
    ...
    for r in races:
        ...
        actual_ticket = str(result.get("trifecta_ticket"))
