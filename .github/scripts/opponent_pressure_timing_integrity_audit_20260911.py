...
def before_cutoff(value: datetime | None) -> bool:
    v = as_jst(value)
    return bool(v is not None and v.date() == TARGET_DATE and v.time().replace(tzinfo=None) <= SOURCE_CUTOFF)
...
def main() -> None:
    ...
    for r in present:
        ...
        c_cut = before_cutoff(created)
        u_cut = before_cutoff(updated)
        ...
        mutable = bool(created is not None and updated is not None and updated > created)
        mutable_rows += int(mutable)
    ...
    print(
        ...
        f"full_safe:{full_safe} model_ok:{model_ok} train_ok:{train_ok}",
        flush=True,
    )
    print(
        "OPP_TIMING_AUDIT_CREATED="
        f"cutoff_ok:{created_cutoff_ok} deadline_ok:{created_deadline_ok} "
        f"post_cutoff:{post_cutoff_creates} post_deadline:{post_deadline_creates}",
        flush=True,
    )
    print(
        "OPP_TIMING_AUDIT_UPDATED="
        f"cutoff_ok:{updated_cutoff_ok} deadline_ok:{updated_deadline_ok} "
        f"mutable:{mutable_rows} post_cutoff:{post_cutoff_updates} post_deadline:{post_deadline_updates}",
        flush=True,
    )
