...
def main() -> None:
    ...
    with psycopg.connect(DATABASE_URL, row_factory=dict_row, autocommit=True) as conn:
        ...
        for r in schema[:120]:
            print(f"MOTOR_METADATA_COLUMN={r['table_name']}.{r['column_name']}:{r['data_type']}", flush=True)

        ecols = {r['column_name'] for r in rows(conn, "select column_name from information_schema.columns where table_schema='public' and table_name='v2_race_entries'")}
        ...
        stats = one(conn, """
            with x as (
              select coalesce(r.venue_id,r.venue_code) as venue_id, e.motor_no::text motor_no,
                     min(r.race_date)::date first_seen, max(r.race_date)::date last_seen,
              ...
              where r.race_date between %s and %s and e.motor_no is not null group by 1,2
            )
            select count(*)::bigint units,
                   ...
                   max(appearances)::bigint max_appearances from x
            """, (START_DATE, END_DATE, START_DATE + timedelta(days=LEFT_CENSOR_DAYS), START_DATE + timedelta(days=LEFT_CENSOR_DAYS)))
        total = int(stats.get('units') or 0); left = int(stats.get('left_censored_units') or 0); post = int(stats.get('post_boundary_units') or 0)
        ...
        print("MOTOR_APPEARANCE_DENSITY=" f"median:{float(stats.get('median_appearances') or 0):.1f} max:{int(stats.get('max_appearances') or 0)} " f"ge10:{int(stats.get('ge10') or 0)} ge30:{int(stats.get('ge30') or 0)} ge60:{int(stats.get('ge60') or 0)}", flush=True)

        bursts = rows(conn, """
            with firsts as (
              select coalesce(r.venue_id,r.venue_code) as venue_id, e.motor_no::text motor_no, min(r.race_date)::date first_seen
              from v2_race_entries e join v2_races r on r.race_id=e.race_id
              where r.race_date between %s and %s and e.motor_no is not null group by 1,2
            ), b as (select venue_id,first_seen,count(*)::bigint new_motor_nos from firsts group by 1,2)
            select venue_id,first_seen,new_motor_nos from b where new_motor_nos >= 10
            order by new_motor_nos desc,first_seen,venue_id limit 80
            """, (START_DATE, END_DATE))
        print(f"MOTOR_FIRST_SEEN_BURSTS={len(bursts)}", flush=True)
        for r in bursts:
            print(f"MOTOR_BURST=venue:{str(r['venue_id']).zfill(2)} date:{r['first_seen']} new_numbers:{int(r['new_motor_nos'])}", flush=True)

        official_like = [r for r in schema if any(k in str(r['column_name']).lower() for k in ('use_start','start_date','replace','exchange')) and ('motor' in str(r['column_name']).lower() or 'motor' in str(r['table_name']).lower())]
        print(f"MOTOR_OFFICIAL_START_LIKE_COLUMNS={len(official_like)}", flush=True)
        if official_like:
            for r in official_like[:40]: print(f"MOTOR_START_SOURCE_CANDIDATE={r['table_name']}.{r['column_name']}", flush=True)
        else:
