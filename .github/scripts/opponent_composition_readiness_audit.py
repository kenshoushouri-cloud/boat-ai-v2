...
# Repository-wide canonical mapping (repair_month_all_pg.py):
# B2=1, B1=2, A2=3, A1=4.
...
def main():
    ...
    print('OPPONENT_CLASS_MAP=B2:1 B1:2 A2:3 A1:4',flush=True)
    with psycopg.connect(DB,row_factory=dict_row,autocommit=True) as conn:
        ...
        if not all(int(schema.get(k) or 0)==1 for k in ('racer_number','racer_class','lane')):
            raise RuntimeError('required v2_race_entries columns missing')

        base=one(conn,"""
          select count(*)::bigint participant_rows,
                 count(distinct e.racer_number)::bigint racers,
                 count(*) filter(where e.racer_class between 1 and 4)::bigint valid_class_rows
        ...
        pair=one(conn,"""
          with g as (
            ...
            where r.race_date between %s and %s and a.racer_number is not null
              ...
              and b.racer_class between 1 and 4 group by 1,2,3,4
          )
        ...
        exact=one(conn,"""
          with x as (
            ...
            join v2_races r on r.race_id=e.race_id
            where r.race_date between %s and %s and e.racer_number is not null and o.racer_class between 1 and 4
            group by 1,2,3
        ...
        agg=one(conn,"""
          with x as (
            select a.race_id,a.racer_number,a.lane,
              count(*) filter(where b.racer_class=4) a1_n,count(*) filter(where b.racer_class=3) a2_n,
              count(*) filter(where b.racer_class=2) b1_n,count(*) filter(where b.racer_class=1) b2_n,
              count(*) filter(where b.lane<a.lane and b.racer_class in (3,4)) inner_a_n,
              count(*) filter(where b.lane>a.lane and b.racer_class in (3,4)) outer_a_n
            from v2_race_entries a join v2_race_entries b on b.race_id=a.race_id and b.lane<>a.lane
            join v2_races r on r.race_id=a.race_id
            where r.race_date between %s and %s and a.racer_number is not null and b.racer_class between 1 and 4
            group by 1,2,3
          ), g as (
