...
Purpose:
- quantify usable sample sizes before creating any prediction coefficient;
- compare fine-grained racer/opponent patterns with safer aggregate opponent-strength patterns;
- identify whether OOS testing is justified.
...
def main():
    ...
    with psycopg.connect(DB,row_factory=dict_row,autocommit=True) as conn:
        ...
        schema=one(conn,"""
          select
            count(*) filter(where column_name='racer_number') racer_number,
        ...
        base=one(conn,"""
          select count(*)::bigint participant_rows,
                 ...
                 count(*) filter(where re.finish_position between 1 and 6)::bigint result_rows
          from v2_race_entries e
          join v2_races r on r.race_id=e.race_id
          left join v2_result_entries re on re.race_id=e.race_id and re.lane=e.lane
        ...
        pair=one(conn,"""
          with g as (
            ...
            where r.race_date between %s and %s
              and a.racer_number is not null
              and a.lane between 1 and 6 and b.lane between 1 and 6
              and b.racer_class in ('A1','A2','B1','B2')
            group by 1,2,3,4
          )
          select count(*)::bigint groups,
                 count(*) filter(where n>=10)::bigint ge10,
                 count(*) filter(where n>=20)::bigint ge20,
                 count(*) filter(where n>=30)::bigint ge30,
                 count(*) filter(where n>=50)::bigint ge50,
                 percentile_cont(.5) within group(order by n)::float8 median_n,
                 percentile_cont(.9) within group(order by n)::float8 p90_n,
                 max(n)::bigint max_n from g
        """,(START,END))
        ...
        exact=one(conn,"""
          with x as (
            select e.race_id,e.racer_number,e.lane,
              max(case when o.lane=1 then o.racer_class end) c1,
              max(case when o.lane=2 then o.racer_class end) c2,
              max(case when o.lane=3 then o.racer_class end) c3,
              max(case when o.lane=4 then o.racer_class end) c4,
              max(case when o.lane=5 then o.racer_class end) c5,
              max(case when o.lane=6 then o.racer_class end) c6
            from v2_race_entries e
            join v2_race_entries o on o.race_id=e.race_id and o.lane<>e.lane
            join v2_races r on r.race_id=e.race_id
            where r.race_date between %s and %s and e.racer_number is not null
              and o.racer_class in ('A1','A2','B1','B2')
            group by 1,2,3
          ), g as (
            select racer_number,lane,c1,c2,c3,c4,c5,c6,count(*) n from x group by 1,2,3,4,5,6,7,8
          )
          select count(*)::bigint groups,
                 count(*) filter(where n>=10)::bigint ge10,
                 count(*) filter(where n>=20)::bigint ge20,
        ...
        agg=one(conn,"""
          with x as (
            select a.race_id,a.racer_number,a.lane,
                   count(*) filter(where b.racer_class='A1') a1_n,
                   count(*) filter(where b.racer_class='A2') a2_n,
                   count(*) filter(where b.racer_class='B1') b1_n,
                   count(*) filter(where b.racer_class='B2') b2_n,
        ...
        lane_class=one(conn,"""
          with g as (
            ...
            join v2_races r on r.race_id=a.race_id
            where r.race_date between %s and %s
              and a.racer_class in ('A1','A2','B1','B2') and b.racer_class in ('A1','A2','B1','B2')
            group by 1,2,3,4
          )
          select count(*)::bigint groups,
                 min(n)::bigint min_n,
