...
create table if not exists v2_bao_market_shadow_snapshots (
 ...
 odds real[] not null,
 exhibition_time_ranks smallint[],
 exhibition_frozen_at timestamptz,
...
def phase_for(minutes_before: float):
    if EARLY_MIN_LO <= minutes_before <= EARLY_MIN_HI:
        return "early"
    if LATE_MIN_LO <= minutes_before <= LATE_MIN_HI:
        return "late"
    return None
...
def parse_official_odds3t(html: str) -> dict[str, float]:
    ...
    for i in range(max(0, len(tokens) - needed + 1)):
        if all(
            lane_token(tokens[i + col * 3], second)
            and lane_token(tokens[i + col * 3 + 1], third)
            ...
            for third_row in range(4):
                for first in firsts:
                    ...
                    if third_row == 0:
                        ...
                        idx += 3
                        if lane_token(second_token, second):
                    ...
                    if ticket in CANONICAL_SET and odd > 0:
                        out[ticket] = odd
        ...
        c.execute(
            """select data_type,udt_name from information_schema.columns
               where table_schema='public'
                 and table_name='v2_bao_market_shadow_snapshots'
                 and column_name='odds'"""
        )
        row = c.fetchone()
        if not row or row["data_type"] != "ARRAY" or row["udt_name"] != "_float4":
            raise RuntimeError(
                "Bao market Shadow table exists with unexpected/noncompact schema"
            )
...
def main():
    ...
    print(
        f"BAO_SHADOW_MODE=isolated_compact_write target:{TARGET_DATE} now:{now.isoformat()}",
        flush=True,
    )
    print(
        f"BAO_SHADOW_WINDOWS=early:{EARLY_MIN_LO}-{EARLY_MIN_HI} "
    ...
    with psycopg.connect(DB, row_factory=dict_row, autocommit=True) as conn:
        ...
        for r, ph, dl in targets:
            ...
            if len(odds) != 120 or set(odds) != CANONICAL_SET:
                partial += 1
                print(
                    f"BAO_SHADOW_SKIP race:{rid} phase:{ph} odds:{len(odds)} "
                    "reason:not_exact120",
                    flush=True,
                )
                continue
            ...
            if captured_phase != ph:
                phase_drift += 1
                print(
                    f"BAO_SHADOW_SKIP race:{rid} phase:{ph} odds:120 "
                    f"reason:phase_drift captured_phase:{captured_phase or 'none'} "
        ...
        with conn.cursor() as c:
            c.execute(
                """select phase,count(*) rows,count(distinct race_id) races,
            ...
            for x in c.fetchall():
                print(
                    "BAO_SHADOW_TOTAL phase:{phase} rows:{rows} races:{races} "
                    "odds_n:{min_n}-{max_n} first:{first_at} last:{last_at}".format(**x),
                    flush=True,
                )
            c.execute(
                """select count(*) paired_races from (
                     select race_id
                     from v2_bao_market_shadow_snapshots
                     group by race_id
                     having count(distinct phase)=2 and count(*)=2
                   ) x"""
            )
            ...
            c.execute(
                "select pg_total_relation_size('v2_bao_market_shadow_snapshots')::bigint bytes"
            )
    ...
    print(
        "BAO_SHADOW_POLICY=isolated_compact_table_only_no_production_decision_change",
        flush=True,
    )
