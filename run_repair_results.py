...
# ============================================================
# 払戻パース(digits方式 + デバッグログ付き)
# ============================================================

def _parse_race_result_debug(html, race_date, jcd, rno):
    soup = BeautifulSoup(html, "html.parser")

    no_data = soup.find(string=lambda t: t and "データがありません" in t)
    if no_data:
        print(f"    -> データがありません表示あり")
        return None
    ...
    if not boats and not payouts["trifecta"]:
        print(f"    -> パース失敗: boats=0, trifecta空 (HTML長={len(html)}文字)")
        return None
