...
Railway Start Command:
    python backtest_multi_patterns_v16_cross_condition_diagnostics_logsafe.py
...
def main() -> None:
    _require_settings()
    print("✅ backtest_multi_patterns_v16_cross_condition_diagnostics_logsafe.py VERSION 2026-06-23 cross-condition-diagnostics-logsafe", flush=True)
    print("=== v16 logsafe カテゴリ×場×開催日クロス診断開始 ===", flush=True)
    ...
    print("\n" + "=" * 92, flush=True)
    print("v16 logsafe カテゴリ×場×開催日クロス診断 最終結果", flush=True)
    print("=" * 92, flush=True)
    print(f"期間: {START_DATE} -> {END_DATE}", flush=True)
