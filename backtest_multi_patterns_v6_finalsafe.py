# -*- coding: utf-8 -*-
"""
backtest_multi_patterns_v6_quiet.py

競艇AI v2用・複数パターン一括バックテスト。

[v6] Railway Variablesを毎回変えずに済むよう、バックテスト条件・focus対象・除外月・追加EV戦略をロジック内で固定。
...
- v4では seed_odds4_25_prob2 を軸に、オッズ帯・人気帯・1号艇A1/A2別の深掘りを行う
...
Railway Start Command:
    python backtest_multi_patterns_v6_quiet.py

Railwayで必要なVariables:
    SUPABASE_URL
    SUPABASE_SERVICE_ROLE_KEY もしくは SUPABASE_KEY
    RAILPACK_PYTHON_VERSION=3.11 推奨

v6では BACKTEST_* の設定変更は不要。
期間・日次上限・focus対象・除外月・追加戦略は RUN_CONFIG と STRATEGIES に内蔵。
...
[修正履歴 2026-06-16]
  ...
  V4: seed_odds4_25_prob2 の派生条件（4〜20、5〜20、A1限定、A2限定、人気1〜5内など）を追加。
"""
...
# v6 Logic Config
# ============================================================
# Railway Variablesで毎回変更しない項目です。
# ここを書き換えれば、Start Commandは常に
#   python backtest_multi_patterns_v6_quiet.py
# のままで検証できます。
...
RUN_CONFIG = {
    ...
    "min_odds": 1.0,
    "max_odds": 9999.0,
    "insurance_min_odds": 6.0,
    "strict_seed": True,

    # 簡易確率モデル設定
    "prob_temp": 2.20,

    # Supabase/Railway実行設定
    ...
    "summary_top_n": 25,
    "print_focus_reports": False,
    "print_csv_paths": False,

    # v6: focus対象を複数固定。Variables変更不要。
    "focus_strategies": [
        "all_prob_2pt",
        "all_prob_2pt_no_v16",
        "all_prob_2pt_no_bad5",
        "all_prob_2pt_odds_5_80",
        "all_prob_2pt_odds_5_100",
        "all_prob_2pt_odds_10_100",
        "all_prob_2pt_odds_10_150",
        "all_ev_5_80_ev105_top2",
        "all_ev_5_100_ev105_top2",
        "all_ev_10_100_ev105_top2",
        "all_ev_10_150_ev105_top2",
        "all_ev_5_100_ev110_top2",
        "seed_A1_odds4_25_prob2",
    ],

    # v6: 一撃依存チェック用。単月除外と複数月除外を自動出力。
    "focus_exclude_months": ["2026-05", "2025-05"],

    # v6: 悪化が目立った場の除外テスト。
    # 直近all_prob_2pt focus結果のbottom寄り: 01,04,05,06,23
...
MIN_EV = float(RUN_CONFIG["min_ev"])
MIN_ODDS = float(RUN_CONFIG["min_odds"])
MAX_ODDS = float(RUN_CONFIG["max_odds"])
INSURANCE_MIN_ODDS = float(RUN_CONFIG["insurance_min_odds"])
STRICT_SEED = bool(RUN_CONFIG["strict_seed"])
WRITE_CSV = bool(RUN_CONFIG["write_csv"])
CSV_DIR = str(RUN_CONFIG["csv_dir"])
...
VERBOSE_LOG = bool(RUN_CONFIG["verbose_log"])
LOG_EVERY_DAYS = int(RUN_CONFIG["log_every_days"])
PRINT_ADOPTED_COUNTS = bool(RUN_CONFIG["print_adopted_counts"])
PRINT_JSON_SUMMARY = bool(RUN_CONFIG["print_json_summary"])
SUMMARY_TOP_N = int(RUN_CONFIG["summary_top_n"])
PRINT_FOCUS_REPORTS = bool(RUN_CONFIG.get("print_focus_reports", False))
PRINT_CSV_PATHS = bool(RUN_CONFIG.get("print_csv_paths", False))

FOCUS_STRATEGIES = [str(x) for x in RUN_CONFIG["focus_strategies"]]
...
# 後方互換用。単一focus前提の古い関数から参照されないよう、v6では基本的に使わない。
FOCUS_STRATEGY = FOCUS_STRATEGIES[0] if FOCUS_STRATEGIES else "all_prob_2pt"
FOCUS_EXCLUDE_MONTH = FOCUS_EXCLUDE_MONTHS[0] if FOCUS_EXCLUDE_MONTHS else "2026-05"
...
def _require_settings() -> None:
    print("✅ backtest_multi_patterns_v6_finalsafe.py VERSION 2026-06-22 no-csv-final-safe", flush=True)
...
class Strategy:
    ...
    # 1号艇級別を限定したい場合。A1=4, A2=3
    lane1_classes: Optional[Tuple[int, ...]] = None
    # 指定場を除外した検証用。例: ("01", "04")
    exclude_venues: Optional[Tuple[str, ...]] = None
...
STRATEGIES: List[Strategy] = [
    ...
    # v6追加: Variables変更なしで耐久検証するための内蔵パターン
    # all_prob_2pt の一撃依存・場依存・オッズ上限依存を確認する。
    Strategy("all_prob_2pt_no_v16", "全候補・確率2点・venue16除外", seed_only=False, main_count=2, score_mode="prob", exclude_venues=("16",)),
    Strategy("all_prob_2pt_no_bad5", "全候補・確率2点・低調5場除外", seed_only=False, main_count=2, score_mode="prob", exclude_venues=BAD_VENUES),
    Strategy("all_prob_2pt_odds_5_80", "全候補・オッズ5〜80倍・確率2点", seed_only=False, main_count=2, score_mode="prob", min_odds=5.0, max_odds=80.0),
    Strategy("all_prob_2pt_odds_5_100", "全候補・オッズ5〜100倍・確率2点", seed_only=False, main_count=2, score_mode="prob", min_odds=5.0, max_odds=100.0),
    Strategy("all_prob_2pt_odds_10_100", "全候補・オッズ10〜100倍・確率2点", seed_only=False, main_count=2, score_mode="prob", min_odds=10.0, max_odds=100.0),
    Strategy("all_prob_2pt_odds_10_150", "全候補・オッズ10〜150倍・確率2点", seed_only=False, main_count=2, score_mode="prob", min_odds=10.0, max_odds=150.0),

    # v6追加: 確率上位固定ではなく EV = prob * odds で選ぶ。
    # min_evは1.05/1.10を内蔵し、買う価値が低い買い目を見送る。
    Strategy("all_ev_5_80_ev105_top2", "全候補・EV1.05以上・オッズ5〜80倍・EV2点", seed_only=False, main_count=2, score_mode="ev", min_ev=1.05, min_odds=5.0, max_odds=80.0),
    Strategy("all_ev_5_100_ev105_top2", "全候補・EV1.05以上・オッズ5〜100倍・EV2点", seed_only=False, main_count=2, score_mode="ev", min_ev=1.05, min_odds=5.0, max_odds=100.0),
    Strategy("all_ev_10_100_ev105_top2", "全候補・EV1.05以上・オッズ10〜100倍・EV2点", seed_only=False, main_count=2, score_mode="ev", min_ev=1.05, min_odds=10.0, max_odds=100.0),
    Strategy("all_ev_10_150_ev105_top2", "全候補・EV1.05以上・オッズ10〜150倍・EV2点", seed_only=False, main_count=2, score_mode="ev", min_ev=1.05, min_odds=10.0, max_odds=150.0),
    Strategy("all_ev_5_100_ev110_top2", "全候補・EV1.10以上・オッズ5〜100倍・EV2点", seed_only=False, main_count=2, score_mode="ev", min_ev=1.10, min_odds=5.0, max_odds=100.0),
    ...
    # v4追加: seed_odds4_25_prob2 深掘り
    Strategy("seed_odds4_20_prob2", "シード戦・1頭固定・オッズ4〜20倍・確率2点", seed_only=True, first_fixed=1, main_count=2, score_mode="prob", min_odds=4.0, max_odds=20.0),
    Strategy("seed_odds5_20_prob2", "シード戦・1頭固定・オッズ5〜20倍・確率2点", seed_only=True, first_fixed=1, main_count=2, score_mode="prob", min_odds=5.0, max_odds=20.0),
    Strategy("seed_odds4_18_prob2", "シード戦・1頭固定・オッズ4〜18倍・確率2点", seed_only=True, first_fixed=1, main_count=2, score_mode="prob", min_odds=4.0, max_odds=18.0),
    Strategy("seed_odds5_25_prob2", "シード戦・1頭固定・オッズ5〜25倍・確率2点", seed_only=True, first_fixed=1, main_count=2, score_mode="prob", min_odds=5.0, max_odds=25.0),
    Strategy("seed_odds4_25_rank1_5_prob2", "シード戦・1頭固定・オッズ4〜25倍・候補内人気1〜5から確率2点", seed_only=True, first_fixed=1, main_count=2, score_mode="prob", min_odds=4.0, max_odds=25.0, fav_rank_min=1, fav_rank_max=5),
    Strategy("seed_A1_odds4_25_prob2", "シード戦・1号艇A1限定・オッズ4〜25倍・確率2点", seed_only=True, first_fixed=1, main_count=2, score_mode="prob", min_odds=4.0, max_odds=25.0, lane1_classes=(4,)),
    Strategy("seed_A2_odds4_25_prob2", "シード戦・1号艇A2限定・オッズ4〜25倍・確率2点", seed_only=True, first_fixed=1, main_count=2, score_mode="prob", min_odds=4.0, max_odds=25.0, lane1_classes=(3,)),
...
def _build_day_candidates(
    ...
    for r in races:
        ...
        for st in STRATEGIES:
            if st.exclude_venues is not None and venue_id in tuple(str(v).zfill(2) for v in st.exclude_venues):
                continue
            if st.seed_only and not is_seed:
                continue
            if st.lane1_classes is not None and lane1_class not in st.lane1_classes:
                continue
...
# ============================================================
# v5 Focus report helpers
# ============================================================

def _summarize_focus_records(records: List[Dict[str, Any]], group_key: str) -> List[Dict[str, Any]]:
    ...
    for rec in records:
        ...
        profit = _safe_int(rec.get("profit_yen"), 0)
        g["races"] += 1
        g["points"] += _safe_int(rec.get("points"), 0)
        g["stake"] += _safe_int(rec.get("stake_yen"), 0)
        g["payout"] += _safe_int(rec.get("payout_yen"), 0)
        g["profit"] += profit
        if rec.get("hit") == 1:
            g["hits"] += 1
            if rec.get("hit_label") == "insurance":
                g["insurance_hits"] += 1
            else:
                g["main_hits"] += 1
        g["best_profit"] = profit if g["best_profit"] is None else max(g["best_profit"], profit)
        g["worst_profit"] = profit if g["worst_profit"] is None else min(g["worst_profit"], profit)
    ...
    for g in grouped.values():
        ...
        hits = g["hits"]
        row = dict(g)
        row["roi"] = round((g["payout"] / stake * 100.0) if stake else 0.0, 2)
        row["hit_rate"] = round((hits / races * 100.0) if races else 0.0, 2)
        row["avg_profit_per_race"] = round((g["profit"] / races) if races else 0.0, 2)
        rows.append(row)

    rows.sort(key=lambda x: (x["roi"], x["profit"]), reverse=True)
    return rows


def _summarize_focus_total(records: List[Dict[str, Any]], label: str) -> Dict[str, Any]:
    races = len(records)
    ...
    main_hits = sum(1 for r in records if r.get("hit") == 1 and r.get("hit_label") != "insurance")
    insurance_hits = sum(1 for r in records if r.get("hit") == 1 and r.get("hit_label") == "insurance")
    max_losing = 0
    cur_losing = 0
    ...
    return {
        ...
        "payout": payout,
        "profit": profit,
        "roi": round((payout / stake * 100.0) if stake else 0.0, 2),
        "hits": hits,
        "hit_rate": round((hits / races * 100.0) if races else 0.0, 2),
        "main_hits": main_hits,
...
def _focus_record_from_rc(rc: RaceCandidate) -> Dict[str, Any]:
    ...
    for b in rc.bets:
        if b["ticket"] == rc.actual_ticket:
            payout = int(rc.actual_payout_yen * UNIT_YEN / PAYOUT_BASE_YEN)
            hit = 1
            hit_label = str(b.get("label", "main"))
    ...
    return {
        ...
        "bet_odds": "|".join(str(b.get("odds", "")) for b in rc.bets),
        "bet_probs": "|".join(str(round(_safe_float(b.get("prob"), 0.0), 6)) for b in rc.bets),
        "bet_evs": "|".join(str(round(_safe_float(b.get("ev"), 0.0), 4)) for b in rc.bets),
        "bet_labels": "|".join(str(b.get("label", "")) for b in rc.bets),
        "is_seed": 1 if rc.is_seed else 0,
    }


def _write_focus_csv(focus_records: List[Dict[str, Any]]) -> None:
    if not WRITE_CSV or not focus_records:
        return

    os.makedirs(CSV_DIR, exist_ok=True)
    stamp = f"{START_DATE}_{END_DATE}".replace("-", "")
    safe_focus = FOCUS_STRATEGY.replace("/", "_")
    detail_path = os.path.join(CSV_DIR, f"focus_{safe_focus}_detail_{stamp}.csv")
    monthly_path = os.path.join(CSV_DIR, f"focus_{safe_focus}_monthly_{stamp}.csv")
    venue_path = os.path.join(CSV_DIR, f"focus_{safe_focus}_venue_{stamp}.csv")
    hits_path = os.path.join(CSV_DIR, f"focus_{safe_focus}_hits_{stamp}.csv")
    total_path = os.path.join(CSV_DIR, f"focus_{safe_focus}_total_{stamp}.csv")

    with open(detail_path, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=list(focus_records[0].keys()))
        w.writeheader()
        w.writerows(focus_records)

    monthly_rows = _summarize_focus_records(focus_records, "month")
    venue_rows = _summarize_focus_records(focus_records, "venue_id")
    hit_rows = [r for r in focus_records if r.get("hit") == 1]
    hit_rows.sort(key=lambda x: _safe_int(x.get("payout_yen"), 0), reverse=True)

    for path, rows in ((monthly_path, monthly_rows), (venue_path, venue_rows), (hits_path, hit_rows)):
        if rows:
    ...
    totals = [
        _summarize_focus_total(focus_records, "all"),
        _summarize_focus_total([r for r in focus_records if r.get("month") != FOCUS_EXCLUDE_MONTH], f"exclude_{FOCUS_EXCLUDE_MONTH}"),
        _summarize_focus_total([r for r in focus_records if r.get("month") == FOCUS_EXCLUDE_MONTH], f"only_{FOCUS_EXCLUDE_MONTH}"),
    ]
    with open(total_path, "w", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=list(totals[0].keys()))
        w.writeheader()
        w.writerows(totals)

    if PRINT_CSV_PATHS:
        print(f"CSV focus detail: {detail_path}", flush=True)
        print(f"CSV focus monthly: {monthly_path}", flush=True)
        print(f"CSV focus venue: {venue_path}", flush=True)
        print(f"CSV focus hits: {hits_path}", flush=True)
        print(f"CSV focus total: {total_path}", flush=True)


def _print_focus_report(focus_records: List[Dict[str, Any]]) -> None:
    print(f"\n--- v5 focus report: {FOCUS_STRATEGY} ---", flush=True)
    if not focus_records:
        print("focus records: 0", flush=True)
        return

    total = _summarize_focus_total(focus_records, "all")
    ex = _summarize_focus_total([r for r in focus_records if r.get("month") != FOCUS_EXCLUDE_MONTH], f"exclude_{FOCUS_EXCLUDE_MONTH}")
    only = _summarize_focus_total([r for r in focus_records if r.get("month") == FOCUS_EXCLUDE_MONTH], f"only_{FOCUS_EXCLUDE_MONTH}")

    for row in (total, ex, only):
        print(
            f"{row['label']:<16} 採用{row['races']:>4}R/{row['points']:>4}点 "
            f"的中{row['hits']:>3}R({row['hit_rate']:>5.1f}%) "
            f"投資{row['stake']:>8,}円 回収{row['payout']:>8,}円 "
            f"損益{row['profit']:>+8,}円 ROI{row['roi']:>6.1f}% "
            f"最大連敗{row['max_losing_streak']:>3}",
            flush=True,
        )

    print("\n--- focus monthly ---", flush=True)
    for r in sorted(_summarize_focus_records(focus_records, "month"), key=lambda x: x["month"]):
        print(
            f"{r['month']} 採用{r['races']:>3}R/{r['points']:>3}点 "
            f"的中{r['hits']:>2}R({r['hit_rate']:>5.1f}%) "
            f"損益{r['profit']:>+7,}円 ROI{r['roi']:>6.1f}%",
            flush=True,
        )

    print("\n--- focus venue top/bottom ---", flush=True)
    venue_rows = _summarize_focus_records(focus_records, "venue_id")
    for r in venue_rows[:8]:
        print(
            f"venue {r['venue_id']} 採用{r['races']:>3}R 的中{r['hits']:>2}R "
            f"損益{r['profit']:>+7,}円 ROI{r['roi']:>6.1f}%",
            flush=True,
        )
    if len(venue_rows) > 8:
        print("...", flush=True)
        for r in venue_rows[-5:]:
            print(
                f"venue {r['venue_id']} 採用{r['races']:>3}R 的中{r['hits']:>2}R "
                f"損益{r['profit']:>+7,}円 ROI{r['roi']:>6.1f}%",
                flush=True,
    ...
    print("\n--- focus hit payout top 10 ---", flush=True)
    hits = [r for r in focus_records if r.get("hit") == 1]
    hits.sort(key=lambda x: _safe_int(x.get("payout_yen"), 0), reverse=True)
    for r in hits[:10]:
        print(
            f"{r['race_id']} {r['venue_id']}R{r['race_no']} "
            f"{r['actual_ticket']} 払戻{r['payout_yen']:,}円 "
            f"買い目[{r['bet_tickets']}] odds[{r['bet_odds']}]",
            flush=True,
        )
...
def _top_hit_excluded_records(records: List[Dict[str, Any]], n: int) -> List[Dict[str, Any]]:
    """的中レコードのうち払戻上位n本だけを除外する。"""
    if n <= 0:
        return records
    hits = [r for r in records if r.get("hit") == 1]
    hits.sort(key=lambda x: _safe_int(x.get("payout_yen"), 0), reverse=True)
    remove_keys = {
        (r.get("race_id"), r.get("actual_ticket"), r.get("payout_yen"))
        for r in hits[:n]
    }
    return [
        r for r in records
        if (r.get("race_id"), r.get("actual_ticket"), r.get("payout_yen")) not in remove_keys
    ]
...
def _write_focus_csv_v6(focus_strategy: str, focus_records: List[Dict[str, Any]]) -> None:
    if not WRITE_CSV or not focus_records:
        return
    ...
    totals: List[Dict[str, Any]] = [_summarize_focus_total(focus_records, "all")]
    for m in FOCUS_EXCLUDE_MONTHS:
        totals.append(_summarize_focus_total(_month_excluded_records(focus_records, [m]), f"exclude_{m}"))
        totals.append(_summarize_focus_total([r for r in focus_records if r.get("month") == m], f"only_{m}"))
    if len(FOCUS_EXCLUDE_MONTHS) >= 2:
        label = "exclude_" + "_".join(FOCUS_EXCLUDE_MONTHS)
        totals.append(_summarize_focus_total(_month_excluded_records(focus_records, FOCUS_EXCLUDE_MONTHS), label))

    for n in (1, 3, 5):
        totals.append(_summarize_focus_total(_top_hit_excluded_records(focus_records, n), f"exclude_top{n}_hit"))
...
def _print_focus_report_v6(focus_strategy: str, focus_records: List[Dict[str, Any]]) -> None:
    if not focus_records:
        print(f"FOCUS {focus_strategy}: records=0", flush=True)
        return

    if not PRINT_FOCUS_REPORTS:
        all_row = _summarize_focus_total(focus_records, "all")
        no_may26 = _summarize_focus_total(_month_excluded_records(focus_records, ["2026-05"]), "exclude_2026-05")
        no_may25 = _summarize_focus_total(_month_excluded_records(focus_records, ["2025-05"]), "exclude_2025-05")
        no_both = _summarize_focus_total(_month_excluded_records(focus_records, FOCUS_EXCLUDE_MONTHS), "exclude_months")
        no_top1 = _summarize_focus_total(_top_hit_excluded_records(focus_records, 1), "exclude_top1_hit")
        print(
            f"FOCUS {focus_strategy:<30} "
            f"allROI={all_row['roi']:>6.1f}% profit={all_row['profit_yen']:>+8,} "
            f"ex26-05={no_may26['roi']:>6.1f}% "
            f"ex25-05={no_may25['roi']:>6.1f}% "
            f"exBoth={no_both['roi']:>6.1f}% "
            f"exTop1={no_top1['roi']:>6.1f}%",
            flush=True,
        )
        return

    print(f"\n--- v6 focus report: {focus_strategy} ---", flush=True)

    rows: List[Dict[str, Any]] = [_summarize_focus_total(focus_records, "all")]
    for m in FOCUS_EXCLUDE_MONTHS:
        rows.append(_summarize_focus_total(_month_excluded_records(focus_records, [m]), f"exclude_{m}"))
        rows.append(_summarize_focus_total([r for r in focus_records if r.get("month") == m], f"only_{m}"))
    if len(FOCUS_EXCLUDE_MONTHS) >= 2:
        label = "exclude_" + "_".join(FOCUS_EXCLUDE_MONTHS)
        rows.append(_summarize_focus_total(_month_excluded_records(focus_records, FOCUS_EXCLUDE_MONTHS), label))
    for n in (1, 3, 5):
        rows.append(_summarize_focus_total(_top_hit_excluded_records(focus_records, n), f"exclude_top{n}_hit"))

    for row in rows:
        print(
            f"{row['label']:<28} 採用{row['races']:>4}R/{row['points']:>4}点 "
            f"的中{row['hits']:>3}R({row['hit_rate']:>5.1f}%) "
            f"投資{row['stake']:>8,}円 回収{row['payout']:>8,}円 "
            f"損益{row['profit']:>+9,}円 ROI{row['roi']:>6.1f}% "
            f"最大連敗{row['max_losing_streak']:>3}",
            flush=True,
        )
    ...
    for r in sorted(_summarize_focus_records(focus_records, "month"), key=lambda x: x["month"]):
        print(
            f"{r['month']} 採用{r['races']:>3}R/{r['points']:>3}点 "
            f"的中{r['hits']:>2}R({r['hit_rate']:>5.1f}%) "
            f"損益{r['profit']:>+8,}円 ROI{r['roi']:>6.1f}%",
            flush=True,
        )
...
def main() -> None:
    ...
    print(f"v6 focus_strategies={','.join(FOCUS_STRATEGIES)}", flush=True)
    print(f"v6 focus_exclude_months={','.join(FOCUS_EXCLUDE_MONTHS)} bad_venues={','.join(BAD_VENUES)}", flush=True)
    ...
    focus_records_by_strategy: Dict[str, List[Dict[str, Any]]] = {name: [] for name in FOCUS_STRATEGIES}
    ...
    for idx, race_date in enumerate(dates, start=1):
        ...
        ms = month_stats.setdefault(month_key, {"races": 0, "ready": 0, "seed": 0})
        ms["races"] += len(races)
        ...
        for st in STRATEGIES:
            ...
            for rc in selected:
                ...
                if rc.strategy in focus_records_by_strategy:
                    focus_records_by_strategy[rc.strategy].append(_focus_record_from_rc(rc))
    ...
    for month_key in sorted(month_stats.keys()):
        ...
        print(
            f"{month_key} races={ms['races']:>5} ready={ms['ready']:>5} seed={ms['seed']:>4}",
            flush=True,
        )
    ...
    for focus_name in FOCUS_STRATEGIES:
        ...
        try:
            ...
            print(f"FOCUS_ERROR {focus_name}: {type(e).__name__}: {e}", flush=True)

    try:
        _write_csv(summary_rows, stats)
    except Exception as e:
        print(f"CSV_WRITE_SKIPPED_BY_ERROR: {type(e).__name__}: {e}", flush=True)
