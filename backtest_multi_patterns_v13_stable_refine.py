...
Railway Start Command:
    python backtest_multi_patterns_v13_stable_refine.py
...
class V13Strategy:
    ...
    # MID: prob_rank4-5 × market_rank21-30 × odds30-50 に対する追加条件
    ...
    # comboでは中穴を優先して日次予算内へ入れるか
    prefer_mid: bool = True


# v13は「連敗短縮」を主目的に、v11/v12の有望条件を実買いロジック化する。
V13_STRATEGIES: List[V13Strategy] = [
    # ---- LOW only: 安定運用の土台 ----
    V13Strategy(
        "low_exR10_12_base",
        "比較基準: 低オッズ 10〜12R除外",
    ...
    V13Strategy(
        "low_exR10_12_no_day5",
        "安定本命: 低オッズ 10〜12R除外 + 5日目除外",
        include_low=True,
        low_filter="exclude_r10_12",
        low_stage_filter="not_day5",
    ),
    V13Strategy(
        "low_exR10_12_no_day4_5",
        "安定比較: 低オッズ 10〜12R除外 + 4〜5日目除外",
        include_low=True,
        low_filter="exclude_r10_12",
        low_stage_filter="not_day4_5",
    ),
    V13Strategy(
        "low_r04_09_no_day5",
        "比較: 低オッズ 4〜9R + 5日目除外",
    ...
    V13Strategy(
        "low_finalday_exR10_12",
        "比較: 低オッズ 終盤日 + 10〜12R除外",
        include_low=True,
        low_filter="exclude_r10_12",
    ...
    # ---- MID only: 中穴は終盤日に絞る ----
    V13Strategy(
        "mid_finalday_nonhead_no_r04_06",
        "中穴安定化: 終盤日 + 1頭除外 + R04〜06除外",
        include_mid=True,
    ...
    V13Strategy(
        "mid_day1_or_finalday_nonhead_no_r04_06",
        "攻撃比較: 初日または終盤日 + 1頭除外 + R04〜06除外",
        include_mid=True,
        mid_stage_filter="day1_or_day6plus",
        mid_race_filter="not_r04_06",
        mid_non_head1=True,
    ),

    # ---- Combo: 低オッズを土台に、中穴を終盤日だけ足す ----
    V13Strategy(
        "combo_low_base_plus_mid_finalday",
        "比較基準: 低オッズexR10〜12 + 中穴終盤日",
        include_low=True,
        include_mid=True,
        low_filter="exclude_r10_12",
        mid_stage_filter="day6plus",
    ...
    V13Strategy(
        "combo_low_no_day5_plus_mid_finalday",
        "本命: 低オッズexR10〜12・5日目除外 + 中穴終盤日",
        include_low=True,
        include_mid=True,
        low_filter="exclude_r10_12",
        low_stage_filter="not_day5",
    ...
    V13Strategy(
        "combo_low_no_day4_5_plus_mid_finalday",
        "安定比較: 低オッズexR10〜12・4〜5日目除外 + 中穴終盤日",
        include_low=True,
        include_mid=True,
        low_filter="exclude_r10_12",
        low_stage_filter="not_day4_5",
    ...
    V13Strategy(
        "combo_low_r04_09_no_day5_plus_mid_finalday",
        "比較: 低オッズR04〜09・5日目除外 + 中穴終盤日",
        include_low=True,
        include_mid=True,
        low_filter="r04_09",
        low_stage_filter="not_day5",
    ...
    V13Strategy(
        "combo_low_no_day5_plus_mid_day1_or_finalday",
        "攻撃比較: 低オッズ5日目除外 + 中穴初日/終盤日",
        include_low=True,
        include_mid=True,
        low_filter="exclude_r10_12",
        low_stage_filter="not_day5",
...
def main() -> None:
    _require_settings()
    print("✅ backtest_multi_patterns_v13_stable_refine.py VERSION 2026-06-23 stable-losing-streak-refine", flush=True)
    print("=== v13 連敗短縮・安定化開始 ===", flush=True)
    ...
    print("対象: v11/v12有望条件を使い、低オッズは5日目除外、中穴は終盤日中心へ絞る。曜日・月内日付は不使用。", flush=True)
    print("目標: 1日1,000円上限で、最大連敗を短くしつつROI105%以上を維持する。", flush=True)
    print("分類: 現DBにrace_name列が無いため、venue連続開催日 + レース番号で初日/終盤R/最終日Rを推定。", flush=True)

    stats: Dict[str, StrategyStats] = {s.name: StrategyStats(s.name, s.description) for s in V13_STRATEGIES}
    records_by_strategy: Dict[str, List[Dict[str, Any]]] = {s.name: [] for s in V13_STRATEGIES}
    ...
    for idx, race_date in enumerate(dates, start=1):
        ...
        day_candidates: Dict[str, List[RaceCandidate]] = {s.name: [] for s in V13_STRATEGIES}
        day_ready = 0
        day_seed = 0
        ...
        if idx == 1 or idx % LOG_EVERY_DAYS == 0 or idx == len(dates):
            adopted = sum(len(day_candidates[s.name]) for s in V13_STRATEGIES)
            print(
    ...
    print("\n" + "=" * 92, flush=True)
    print("v13 連敗短縮・安定化 最終結果", flush=True)
    print("=" * 92, flush=True)
    print(f"期間: {START_DATE} -> {END_DATE}", flush=True)
    ...
    print("--- v13安定化戦略サマリー ROI順 ---", flush=True)
    rows = [stats[s.name].row() for s in V13_STRATEGIES]
    rows.sort(key=lambda r: r["roi"], reverse=True)
    for r in rows:
        _print_total_row(r)

    print("\n--- v13 ステージ別確認 ---", flush=True)
    for r in rows:
        _print_stage_diagnostics(r["strategy"], records_by_strategy.get(r["strategy"], []))

    print("\n=== v13 連敗短縮・安定化終了 ===", flush=True)
