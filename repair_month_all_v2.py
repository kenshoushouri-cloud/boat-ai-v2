...
def _require_settings() -> None:
    print("✅ repair_month_all_v2.py VERSION 2026-06-13 no-priority", flush=True)
...
def _missing_column_from_pgrst204(text: str) -> Optional[str]:
    """PostgREST schema cache errorから存在しない列名を取り出す。"""
...
def _rest_post(table: str, rows: List[Dict[str, Any]], on_conflict: str) -> int:
    ...
    for _attempt in range(8):
        ...
        body = res.text or ""
        missing_col = _missing_column_from_pgrst204(body)
        if res.status_code == 400 and "PGRST204" in body and missing_col:
