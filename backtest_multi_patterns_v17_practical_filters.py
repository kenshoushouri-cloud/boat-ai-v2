...
Railway Start Command:
    python backtest_multi_patterns_v17_practical_filters.py
...
# v17はv16クロス診断で良かった条件を実買い戦略として検証する。
# 目的:
# - low_exR10_12_base を会場タイプ/開催日/レース帯で絞り、ROIと最大連敗を改善する
# - category条件は件数が少ないため、まずは比較扱い
V17_STRATEGIES: List[V17Strategy] = [
    ...
    # ---- 開催日×レース帯 ----
    V17Strategy(
        "low_cross_day_race_best",
        "低オッズ: day2_3×R04_06/R07_09 + day6plus×R01_03/R07_09",
        include_low=True,
        low_filter="exclude_r10_12",
    ...
    V17Strategy(
        ...
        include_low=True,
        low_filter="r01_03",
        low_stage_filter="day6plus",
    ),
    V17Strategy(
        "low_day2_3_or_6plus_r07_09",
        "低オッズ: 2〜3日目/終盤日 × R07〜09",
        include_low=True,
        low_filter="r07_09",
        low_stage_filter="day2_3_or_day6plus",
    ),
    ...
    V17Strategy(
        ...
        include_low=True,
        low_filter="r01_03",
        extra_filter="venue_instrong",
    ),
    V17Strategy(
        "low_venue_best_combo",
        "低オッズ: bad5×R04〜09 + イン強×R01〜03/R07〜09",
        include_low=True,
        low_filter="exclude_r10_12",
    ...
    # ---- カテゴリ×会場タイプ×レース帯。件数少なめなので比較扱い ----
    V17Strategy(
        "low_general_cup_bad5_r04_09",
        "低オッズ: 一般カップ系 × bad5 × R04〜09",
        include_low=True,
        low_filter="r04_09",
        extra_filter="general_cup_bad5",
    ),
    V17Strategy(
        "low_category_ladies_rookie",
        "低オッズ: ヴィーナス/女子/ルーキー系",
        include_low=True,
        low_filter="exclude_r10_12",
...
def _print_cross_table(title: str, records: List[Dict[str, Any]], fields: List[str], min_n: int = 5, limit: int = 8) -> None:
...
def _print_stage_diagnostics(strategy: str, records: List[Dict[str, Any]]) -> None:
    ...
    _print_cross_table("開催日×レース番号帯", records, ["event_day_broad", "racegrp"], min_n=5, limit=8)
    _print_cross_table("会場タイプ×レース番号帯", records, ["meta_venue_style", "racegrp"], min_n=5, limit=8)
    _print_cross_table("イベントカテゴリ×レース番号帯", records, ["meta_event_category", "racegrp"], min_n=3, limit=8)
    _print_cross_table("イベントカテゴリ×会場タイプ×レース帯", records, ["meta_event_category", "meta_venue_style", "racegrp"], min_n=3, limit=8)


def main() -> None:
    _require_settings()
    print("✅ backtest_multi_patterns_v17_practical_filters.py VERSION 2026-06-23 practical-cross-filters", flush=True)
    print("=== v17 実運用候補クロス条件検証開始 ===", flush=True)
    ...
    print("対象: v16クロス診断の有望条件を、実運用フィルターとして検証。", flush=True)
    print("目的: low_exR10_12_baseよりROI/最大連敗が良い通常モード候補を作る。", flush=True)
    print("分類: 現DBにrace_name列が無いため、venue連続開催日 + レース番号で初日/終盤R/最終日Rを推定。", flush=True)

    stats: Dict[str, StrategyStats] = {s.name: StrategyStats(s.name, s.description) for s in V17_STRATEGIES}
    records_by_strategy: Dict[str, List[Dict[str, Any]]] = {s.name: [] for s in V17_STRATEGIES}
    ...
    for idx, race_date in enumerate(dates, start=1):
        ...
        day_candidates: Dict[str, List[RaceCandidate]] = {s.name: [] for s in V17_STRATEGIES}
        day_ready = 0
        day_seed = 0
        ...
        if idx == 1 or idx % LOG_EVERY_DAYS == 0 or idx == len(dates):
            adopted = sum(len(day_candidates[s.name]) for s in V17_STRATEGIES)
            print(
    ...
    print("\n" + "=" * 92, flush=True)
    print("v17 実運用候補クロス条件検証 最終結果", flush=True)
    print("=" * 92, flush=True)
    print(f"期間: {START_DATE} -> {END_DATE}", flush=True)
    ...
    if not meta_samples:
        ...
        for k in sorted(meta_samples):
            print(f"{k}: " + " | ".join(meta_samples[k][:3]), flush=True)
    ...
    print("--- v16 logsafe 戦略サマリー ROI順 ---", flush=True)
    rows = [stats[s.name].row() for s in V17_STRATEGIES]
    rows.sort(key=lambda r: r["roi"], reverse=True)
    for r in rows:
        _print_total_row(r)

    print("\n--- v17 条件別確認（主要戦略のみ） ---", flush=True)
    for r in rows:
        if r["strategy"] in FOCUS_DIAG_STRATEGIES:
            _print_stage_diagnostics(r["strategy"], records_by_strategy.get(r["strategy"], []))

    print("\n=== v17 実運用候補クロス条件検証終了 ===", flush=True)
