...
Railway Start Command:
    python v22_realtime_decision_engine.py
...
def main() -> None:
    _require_settings()
    print("✅ v22_realtime_decision_engine.py VERSION 2026-06-23 realtime-decision-engine", flush=True)
    print("=== v22 直前判定エンジン開始 ===", flush=True)
    print(
        f"TARGET_DATE={TARGET_DATE} SNAPSHOT_LABEL={SNAPSHOT_LABEL} DECISION_LABEL={DECISION_LABEL} "
        f"SELECTOR_MODE={SELECTOR_MODE} REQUIRE_EXHIBITION={REQUIRE_EXHIBITION}",
        flush=True,
    )
    ...
    print(f"candidate_rows={len(candidate_rows)} skipped_not_ready={skipped_not_ready}", flush=True)
