...
Railway Start Command:
    python backtest_multi_patterns_v16_cross_condition_diagnostics.py
...
def _collect_meta_samples(row: Dict[str, Any], samples: Dict[str, List[str]], limit: int = 8) -> None:
    for k in META_TEXT_KEYS:
        s = _normalize_jp_text(row.get(k))
        if not s:
            continue
...
# v15はグレード/女子/一般カテゴリの影響を見る診断。
# まずはv14で有望な通常モード + 馬王候補だけに絞る。
V15_STRATEGIES: List[V15Strategy] = [
    V15Strategy(
        "low_exR10_12_base",
        "通常基準: 低オッズ 10〜12R除外",
        include_low=True,
    ...
    V15Strategy(
        ...
        "通常厳選: 低オッズ 10〜12R除外 + 2〜3日目 or 終盤日",
        include_low=True,
        low_filter="exclude_r10_12",
...
def _print_cross_table(title: str, records: List[Dict[str, Any]], fields: List[str], min_n: int = 5, limit: int = 25) -> None:
...
def _print_stage_diagnostics(strategy: str, records: List[Dict[str, Any]]) -> None:
    ...
    _print_group_table("メタ情報有無", records, "meta_info", min_n=5)
    _print_group_table("グレード別", records, "meta_grade", min_n=5)
    _print_group_table("女子/男女混合別", records, "meta_gender", min_n=5)
    _print_group_table("イベントカテゴリ別", records, "meta_event_category", min_n=5)
    _print_group_table("session_type別", records, "meta_session", min_n=5)
    _print_group_table("会場タイプ別", records, "meta_venue_style", min_n=5)
    _print_group_table("イベントカテゴリ×会場タイプ", records, "meta_event_venue", min_n=5)
    _print_group_table("グレード×会場タイプ", records, "meta_grade_venue", min_n=5)
    _print_group_table("女子カテゴリ×イベントカテゴリ", records, "meta_gender_event", min_n=5)
    ...
    # v16: 単独分類ではなく、カテゴリ×場×開催日×レース帯で候補を探す。
    _print_cross_table("開催日×レース番号帯", records, ["event_day_broad", "racegrp"], min_n=5)
    _print_cross_table("stage_combo×レース番号帯", records, ["stage_combo", "racegrp"], min_n=5)
    _print_cross_table("session×レース番号帯", records, ["meta_session", "racegrp"], min_n=5)
    _print_cross_table("会場タイプ×レース番号帯", records, ["meta_venue_style", "racegrp"], min_n=5)
    _print_cross_table("会場タイプ×開催日", records, ["meta_venue_style", "event_day_broad"], min_n=5)
    _print_cross_table("イベントカテゴリ×レース番号帯", records, ["meta_event_category", "racegrp"], min_n=3)
    _print_cross_table("イベントカテゴリ×開催日", records, ["meta_event_category", "event_day_broad"], min_n=3)
    _print_cross_table("イベントカテゴリ×会場タイプ×レース帯", records, ["meta_event_category", "meta_venue_style", "racegrp"], min_n=3)
    _print_cross_table("女子カテゴリ×会場タイプ×レース帯", records, ["meta_gender", "meta_venue_style", "racegrp"], min_n=3)
    _print_cross_table("グレード×会場タイプ×レース帯", records, ["meta_grade", "meta_venue_style", "racegrp"], min_n=3)


def main() -> None:
    _require_settings()
    print("✅ backtest_multi_patterns_v16_cross_condition_diagnostics.py VERSION 2026-06-23 cross-condition-diagnostics", flush=True)
    print("=== v16 カテゴリ×場×開催日クロス診断開始 ===", flush=True)
    ...
    print("対象: v14有望条件に対し、カテゴリ×場×開催日×レース帯のクロス条件を診断。", flush=True)
    print("注意: race_title補修後の実行を前提。単独カテゴリではなく複合条件の候補を探す。", flush=True)
    print("分類: 現DBにrace_name列が無いため、venue連続開催日 + レース番号で初日/終盤R/最終日Rを推定。", flush=True)

    stats: Dict[str, StrategyStats] = {s.name: StrategyStats(s.name, s.description) for s in V15_STRATEGIES}
    records_by_strategy: Dict[str, List[Dict[str, Any]]] = {s.name: [] for s in V15_STRATEGIES}
    monthly: Dict[str, Dict[str, int]] = {}
    meta_samples: Dict[str, List[str]] = {}
    ...
    for idx, race_date in enumerate(dates, start=1):
        ...
        day_candidates: Dict[str, List[RaceCandidate]] = {s.name: [] for s in V15_STRATEGIES}
        day_ready = 0
        day_seed = 0
        ...
        for race in races:
            ...
            _collect_meta_samples(race, meta_samples)
            meta_text = _metadata_text(race)
            meta_info = _meta_info_status(meta_text)
            ...
            meta_grade_event = f"{meta_grade}/{meta_event_category}"
            meta_gender_event = f"{meta_gender}/{meta_event_category}"
            meta_event_venue = f"{meta_event_category}/{meta_venue_style}"
            meta_grade_venue = f"{meta_grade}/{meta_venue_style}"
            ...
            for st in V15_STRATEGIES:
                ...
                rc = RaceCandidate(
                    ...
                    meta_event_category=meta_event_category,
                    meta_session=meta_session,
                    meta_venue_style=meta_venue_style,
                    meta_grade_event=meta_grade_event,
                    meta_gender_event=meta_gender_event,
                    meta_event_venue=meta_event_venue,
                    meta_grade_venue=meta_grade_venue,
                    strategy=st.name,
                    bets=bets,
        ...
        if idx == 1 or idx % LOG_EVERY_DAYS == 0 or idx == len(dates):
            adopted = sum(len(day_candidates[s.name]) for s in V15_STRATEGIES)
            print(
    ...
    print("\n" + "=" * 92, flush=True)
    print("v16 カテゴリ×場×開催日クロス診断 最終結果", flush=True)
    print("=" * 92, flush=True)
    print(f"期間: {START_DATE} -> {END_DATE}", flush=True)
    ...
    print("--- メタデータ列サンプル ---", flush=True)
    if not meta_samples:
        print("タイトル/グレード/カテゴリ候補列は見つかりませんでした。全カテゴリは unknown 扱いです。", flush=True)
    else:
        for k in sorted(meta_samples):
            print(f"{k}: " + " | ".join(meta_samples[k][:5]), flush=True)
    ...
    print("--- v16クロス診断戦略サマリー ROI順 ---", flush=True)
    rows = [stats[s.name].row() for s in V15_STRATEGIES]
    rows.sort(key=lambda r: r["roi"], reverse=True)
    for r in rows:
        _print_total_row(r)

    print("\n--- v16 クロス条件別確認 ---", flush=True)
    for r in rows:
        _print_stage_diagnostics(r["strategy"], records_by_strategy.get(r["strategy"], []))

    print("\n=== v16 カテゴリ×場×開催日クロス診断終了 ===", flush=True)
