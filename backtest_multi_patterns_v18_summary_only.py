...
Railway Start Command:
    python backtest_multi_patterns_v18_summary_only.py
...
def main() -> None:
    _require_settings()
    print("✅ backtest_multi_patterns_v18_summary_only.py VERSION 2026-06-23 final-candidate-summary-only", flush=True)
    print("=== v18 summary-only 最終候補モード比較開始 ===", flush=True)
    ...
    print("対象: v17結果から通常モード最終候補を比較。", flush=True)
    print("目的: base / 厳選 / 広め / union / intersection のどれを通常モードにするか決める。", flush=True)
    ...
    print("\n" + "=" * 92, flush=True)
    print("v18 summary-only 最終候補モード比較 最終結果", flush=True)
    print("=" * 92, flush=True)
    print(f"期間: {START_DATE} -> {END_DATE}", flush=True)
    ...
    for r in rows:
        if r["strategy"] in focus:
            _print_focus(r["strategy"], records_by_strategy.get(r["strategy"], []))

    print("\n=== v18 summary-only 最終候補モード比較終了 ===", flush=True)
