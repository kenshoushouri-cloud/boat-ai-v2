...
Railway Start Command:
    python backtest_multi_patterns_v15_meta_category_diagnostics.py
...
def _infer_grade(meta_text: str) -> str:
    ...
    # SGはG1等より先に見る
    if "SG" in t or "スペシャルグレード" in t:
        return "SG"
    if "G1" in t or "GⅠ" in t or "GI" in t:
        return "G1"
...
def _infer_event_category(meta_text: str) -> str:
    ...
    if "ルーキー" in t:
        return "rookie"
    if "マスターズ" in t or "匠" in t:
        return "masters"
    if "オールレディース" in t or "ヴィーナス" in t or "レディース" in t or "女子" in t:
        return "ladies"
    if "一般" in t:
...
def _meta_info_status(meta_text: str) -> str:
    return "meta_known" if _normalize_jp_text(meta_text) else "meta_unknown"


def _collect_meta_samples(row: Dict[str, Any], samples: Dict[str, List[str]], limit: int = 5) -> None:
    for k in META_TEXT_KEYS:
        s = _normalize_jp_text(row.get(k))
        if not s:
            continue
...
def main() -> None:
    _require_settings()
    print("✅ backtest_multi_patterns_v15_meta_category_diagnostics.py VERSION 2026-06-23 grade-women-general-diagnostics", flush=True)
    print("=== v15 グレード・女子・一般カテゴリ診断開始 ===", flush=True)
    ...
    print("対象: v14有望条件に対し、グレード・女子・一般カテゴリ別の成績を診断。", flush=True)
    print("注意: v2_racesにタイトル/グレード列が無い場合、カテゴリはunknownになる。", flush=True)
    ...
    print("\n" + "=" * 92, flush=True)
    print("v15 グレード・女子・一般カテゴリ診断 最終結果", flush=True)
    print("=" * 92, flush=True)
    print(f"期間: {START_DATE} -> {END_DATE}", flush=True)
