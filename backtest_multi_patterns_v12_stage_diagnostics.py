...
Railway Start Command:
    python backtest_multi_patterns_v12_stage_diagnostics.py
...
# ============================================================
# v12 Stage diagnostics
# ============================================================

# v12では v2_races.race_name を使って、準優勝戦・優勝戦・予選・一般戦などを分類する。
# ただし race_name 列がまだない環境でも落ちないよう、fallback select も用意する。
def _fetch_day_rows(date_str: str) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]], Dict[str, List[Dict[str, Any]]], Dict[str, Dict[str, float]]]:
    day_prefix = _rid_prefix(date_str)
    next_prefix = _rid_prefix(_next_day(date_str))
    ...
    try:
        races = _rest_get(
            "v2_races",
            {
                "select": "race_id,race_date,venue_id,race_no,race_name",
                "race_date": f"eq.{date_str}",
                "order": "venue_id.asc,race_no.asc",
            },
        )
    except Exception as e:
        print(f"  [fallback] v2_races.race_name select failed: {e}", flush=True)
        races = _rest_get(
            "v2_races",
            {
                "select": "race_id,race_date,venue_id,race_no",
...
def _stage_combo(title_stage: str, day_no: int) -> str:
    # レース名が強い場合はそれを優先。初日などは開催日数から補助分類。
    if title_stage in ("semifinal", "final", "dream", "selection", "general", "qualifying"):
        return title_stage
    if day_no == 1:
        return "event_day1_unknown_title"
    return title_stage
...
def main() -> None:
    _require_settings()
    print("✅ backtest_multi_patterns_v12_stage_diagnostics.py VERSION 2026-06-23 stage-diagnostics", flush=True)
    ...
    print("分類: race_name由来の準優/優勝/予選/一般 + venue連続開催日からの初日/2-3日目/4-5日目/6日目以降", flush=True)
    ...
    for idx, race_date in enumerate(dates, start=1):
        ...
        yesterday = _next_day(race_date, -1)
        ...
        for race in races:
            ...
            combo_stage = _stage_combo(title_stage, event_day_no)
