...
Railway Start Command:
    python v24_pre_candidate_notifier.py
...
def _session_match(session: str) -> bool:
    s = (session or "day").lower()
    if PRE_SESSION == "all":
        return True
    if PRE_SESSION == "night":
        return s in ("night", "midnight")
    # day
    return s not in ("night", "midnight")
...
def main() -> None:
    _require_settings()
    print("✅ v24_pre_candidate_notifier.py VERSION 2026-06-25 pre-candidate-notifier", flush=True)
    ...
    for race in races:
        ...
        if not _session_match(meta_session):
            skipped_session += 1
            continue
    ...
    print(f"races={len(races)} candidates={len(rows_out)} selected={len(selected)} skipped_not_ready={skipped_not_ready} skipped_session={skipped_session}", flush=True)
