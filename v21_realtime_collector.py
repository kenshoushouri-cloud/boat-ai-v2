...
Railway Start Command:
    python v21_realtime_collector.py
...
def parse_exhibition(html: str) -> List[Dict[str, Any]]:
    ...
    for cells in rows:
        ...
        # どちらも無い行は選手行ではない可能性が高い
        if ex_time is None and st is None:
            continue
        course = _find_exhibition_course(cells, lane)
    ...
    # rank/diff
    times = [(r["lane"], r["exhibition_time"]) for r in out if r.get("exhibition_time") is not None]
    if times:
        sorted_times = sorted(times, key=lambda x: x[1])
        best = sorted_times[0][1]
        rank = {lane: i + 1 for i, (lane, _) in enumerate(sorted_times)}
        for r in out:
            if r.get("exhibition_time") is not None:
                r["exhibition_time_rank"] = rank.get(r["lane"])
                r["exhibition_time_diff"] = round(float(r["exhibition_time"]) - float(best), 3)

    sts = [(r["lane"], r["start_timing"]) for r in out if r.get("start_timing") is not None]
    if sts:
        sorted_st = sorted(sts, key=lambda x: x[1])
        best = sorted_st[0][1]
        rank = {lane: i + 1 for i, (lane, _) in enumerate(sorted_st)}
        for r in out:
            if r.get("start_timing") is not None:
                r["start_timing_rank"] = rank.get(r["lane"])
                r["start_timing_diff"] = round(float(r["start_timing"]) - float(best), 3)
...
def save_exhibition_and_entries(race: Dict[str, Any], entries: List[Dict[str, Any]], exh: List[Dict[str, Any]]) -> Tuple[int, int]:
    ...
    for r in exh:
        ...
        exh_rows.append({
            ...
            "source": "official_beforeinfo",
            "lane": lane,
            "exhibition_course": r.get("exhibition_course") or lane,
        ...
        course = r.get("exhibition_course") or lane
        entry_rows.append({
            "race_id": rid,
            "race_date": race.get("race_date"),
            "venue_id": venue_id,
...
def main() -> None:
    _require_settings()
    print("✅ v21_realtime_collector.py VERSION 2026-06-23 realtime-collector", flush=True)
    print(f"TARGET_DATE={TARGET_DATE} SNAPSHOT_LABEL={SNAPSHOT_LABEL} SCOPE={COLLECT_SCOPE} TARGET_RACE_ID={TARGET_RACE_ID or '-'}", flush=True)
    ...
    for idx, race in enumerate(target_races, start=1):
        ...
        print(
            f"[{idx}/{len(target_races)}] {rid} before={'ok' if before_html else 'ng'} odds={len(odds)} source={source if odds else '-'}",
            flush=True,
        )
