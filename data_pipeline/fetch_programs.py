...
# 既存の通常対象場
# 通常ジョブで venue_ids を渡さない場合はこの5場を使う
...
def _normalize_venues(venue_ids=None):
    """
    venue_ids が指定された場合はそれを使う。
    未指定なら既存の TARGET_VENUES を使う。
    """
...
def _parse_racelist(html, hd, jcd, rno):
    ...
    while i < len(data_trs):
        ...
        try:
            ...
            boat_no = int(
                tds[0].get_text(strip=True).translate(
    ...
    try:
        ...
        time_str = times[int(rno) - 1] if int(rno) <= len(times) else None
...
def fetch_programs_api(target_date, venue_ids=None):
    """
    出走表取得。
    ...
    for row in rows[:2]:
        ...
        for b in row["boats"]:
            print(
                ...
                "motor=",
                b["racer_motor_number"],
                b["racer_motor_place2_rate"],
            )