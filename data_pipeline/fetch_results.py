...
```
# ページに「データがありません」が含まれる場合
no_data = soup.find(string=lambda t: t and "データがありません" in t)
if no_data:
    # [修正①] no_data と parse失敗を区別してログ出力
    print(f"    → 「データがありません」表示あり")
    return None
...
# --- 着順パース ---
boats = []
seen_places = set()  # [修正②] 重複排除: 同じ着順が複数テーブルから拾われるのを防ぐ
    ...
    for tr in table.find_all("tr"):
        ...
        try:
            ...
            if (
                1 <= place_no <= 6
                and 1 <= boat_no <= 6
                and place_no not in seen_places  # [修正②] 重複チェック
            ):
...
if not boats and not payouts["trifecta"]:
    # [修正①] パース失敗の詳細をログに残す（桐生R8-R12の原因特定用）
