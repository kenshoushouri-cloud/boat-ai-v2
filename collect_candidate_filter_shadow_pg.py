...
- 購入処理はありません。
- 同一race_id・rule_id・ticketはUPSERTします。
"""
...
RULES = [
    {
        ...
        "pr_min": 6, "pr_max": 15, "mr_min": 21, "mr_max": 30,
        "odds_min": 30.0, "odds_max": 50.0,
        "race_nos": set(range(1, 10)),
        "venue_style": "standard", "event_category": "ALL",
        "select_mode": "ev",
    },
    {
        "rule_id": "S02",
        "description": "pr16-30 mr6-10 odds20-30 R07-09 in_strong prob",
        "pr_min": 16, "pr_max": 30, "mr_min": 6, "mr_max": 10,
        "odds_min": 20.0, "odds_max": 30.0,
        "race_nos": {7, 8, 9},
        "venue_style": "in_strong", "event_category": "ALL",
        "select_mode": "prob",
    },
    {
        "rule_id": "S03",
        "description": "pr11-25 mr6-10 odds30-50 R07-09 standard EV",
        "pr_min": 11, "pr_max": 25, "mr_min": 6, "mr_max": 10,
        "odds_min": 30.0, "odds_max": 50.0,
        "race_nos": {7, 8, 9},
        "venue_style": "standard", "event_category": "ALL",
        "select_mode": "ev",
    },
    {
        ...
        "pr_min": 1, "pr_max": 5, "mr_min": 11, "mr_max": 20,
        "odds_min": 20.0, "odds_max": 30.0,
        "race_nos": {1, 2, 3},
        "venue_style": "ALL", "event_category": "ALL",
        "select_mode": "ev",
    },
    {
        "rule_id": "S05",
        "description": "pr1-5 mr1-5 odds10-20 all_ladies prob",
        "pr_min": 1, "pr_max": 5, "mr_min": 1, "mr_max": 5,
        "odds_min": 10.0, "odds_max": 20.0,
        "race_nos": set(range(1, 13)),
        "venue_style": "ALL", "event_category": "all_ladies",
        "select_mode": "prob",
    },
]
...
def _ensure_schema() -> None:
    ddl = [
        "create table if not exists v2_candidate_filter_shadow (id bigserial primary key);",
        ...
        "create unique index if not exists uq_v2_candidate_filter_shadow_main on v2_candidate_filter_shadow (race_id, rule_id, ticket);",
        "create index if not exists ix_v2_candidate_filter_shadow_date on v2_candidate_filter_shadow (race_date, rule_id);",
    ]
    for sql in ddl:
        execute(sql)
...
def _fetch_day_rows():
    ...
    if target_ids:
        races = [r for r in races if str(r.get("race_id") or "") in target_ids]

    entries = fetch_all(
        "select * from v2_race_entries where race_id >= %s and race_id < %s order by race_id,lane;",
...
def _select_one(matches: List[Dict[str, Any]], mode: str) -> Optional[Dict[str, Any]]:
    if not matches:
        return None
    if mode == "ev":
        return max(matches, key=lambda r: (_safe_float(r.get("raw_ev")), _safe_float(r.get("prob"))))
    return max(matches, key=lambda r: (_safe_float(r.get("prob")), _safe_float(r.get("raw_ev"))))


def main() -> None:
    print("✅ collect_candidate_filter_shadow_pg.py VERSION 2026-08-01 initial-five-rules-v1", flush=True)
    print(
        f"TARGET_DATE={TARGET_DATE} WINDOW_NAME={WINDOW_NAME} ENABLED={ENABLED} REQUIRE_COMPLETE_ODDS={REQUIRE_COMPLETE_ODDS}",
        flush=True,
    )
    print("Shadow保存のみ。LINE通知・本番判定・購入処理は変更しません。", flush=True)
    ...
    ready_races = skipped_entries = skipped_odds = 0
    matched_by_rule = {r["rule_id"]: 0 for r in RULES}
    ...
    for race in races:
        ...
        for rule in RULES:
            ...
            if rule["event_category"] != "ALL" and event_category != rule["event_category"]:
                continue
            selected = _select_one([r for r in ranked if _match_rule(r, rule)], rule["select_mode"])
            if not selected:
                continue
            ...
            rows_out.append({
                ...
                "investment_yen": 100,
                "updated_at": now_iso,
                "raw": {"rule": {k: v for k, v in rule.items() if k != "race_nos"}, "window_name": WINDOW_NAME},
            })
    ...
    if rows_out:
        saved = upsert_rows(
            ...
            rows_out,
            ["race_id", "rule_id", "ticket"],
        )

    print(
        f"races={len(races)} ready_races={ready_races} skipped_entries={skipped_entries} skipped_odds={skipped_odds}",
        flush=True,
    )
    print(f"candidate_rows={len(rows_out)} saved_rows={saved}", flush=True)
