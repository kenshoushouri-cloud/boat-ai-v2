...
def main() -> None:
    _require_settings()
    print("✅ daily_selector_v19.py VERSION 2026-06-23 daily-selector-balanced", flush=True)
    print("=== v19 当日買い目出力開始 ===", flush=True)
    print(f"TARGET_DATE={TARGET_DATE} SELECTOR_MODE={SELECTOR_MODE} unit={UNIT_YEN}円 daily_budget={DAILY_BUDGET_YEN}円", flush=True)
    print("通常本命: mode_balanced_venue_best / 強化: intersection・general_cup_bad5・strict_bad5", flush=True)
    ...
    if not races:
        print("対象日のv2_racesがありません。先に月次/日次補修でレース・出走表・オッズを保存してください。", flush=True)
        print("=== v19 当日買い目出力終了 ===", flush=True)
        return
    ...
    for race in races:
        ...
        if not _is_live_ready(entries, odds):
            skipped_not_ready += 1
            continue
    ...
    selected = rows_out[:max_points]

    print(f"live_ready_candidates={len(rows_out)} skipped_not_ready={skipped_not_ready}", flush=True)
    print(f"selected={len(selected)}点 / max_points={max_points}", flush=True)

    if not selected:
        print("本日の条件一致買い目はありません。見送りです。", flush=True)
        print("=== v19 当日買い目出力終了 ===", flush=True)
        return
    ...
    for i, r in enumerate(selected, start=1):
        modes = ",".join(r["modes"])
        print(
            f"{i:02d}. {r['mode_label']} {r['race_id']} {r['venue_id']}場 {r['race_no']}R "
            f"{r['ticket']} odds={r['odds']:.1f} "
            f"prob_rank={r['prob_rank']} market_rank={r['market_rank']} "
            f"event_day={r['event_day_no']} {r['racegrp']} venue={r['venue_style']} "
            f"cat={r['event_category']} grade={r['grade']} modes={modes}",
            flush=True,
        )
        if r["race_title"]:
            print(f"    title: {r['race_title']}", flush=True)
        print(f"    entries: {r['entries']}", flush=True)

    print("\n--- モード説明 ---", flush=True)
    print("通常本命: mode_balanced_venue_best = bad5×R04〜09 + in_strong×R01〜03/R07〜09", flush=True)
    print("強化/厳選: intersection / general_cup_bad5 / strict_bad5 条件", flush=True)
    print("広め候補: SELECTOR_MODE=wide の時に union/not_standard も表示", flush=True)
    print("=== v19 当日買い目出力終了 ===", flush=True)
